...
if __name__ == '__main__':
  ...
  if args.cuda:
    ...
    ss_boxes = ss_boxes.cuda()

  # make variable
  im_data = Variable(im_data)
  im_info = Variable(im_info)
  num_boxes = Variable(num_boxes)
  ss_boxes = Variable(ss_boxes)
  ...
  for epoch in range(args.start_epoch, args.max_epochs):
    ...
    for step in range(iters_per_epoch):
      ...
      ss_boxes.data.resize_(data[2].size()).copy_(data[2])
      num_boxes.data.resize_(data[3].size()).copy_(data[3])
      im_label = Variable(data[4].view(-1))
      if args.cuda:
          im_label = im_label.cuda()

      FPN.zero_grad()
      _, wsddn_loss_cls, wsddn_loss_bbox = FPN(im_data, im_info, ss_boxes, num_boxes, im_label)

      loss = wsddn_loss_cls.mean() + wsddn_loss_bbox.mean()
      ...
      if step % args.disp_interval == 0:
        ...
        if args.mGPUs:
          ...
          loss_wsddn_cls = wsddn_loss_cls.mean().data[0]
          loss_wsdnn_box = wsddn_loss_bbox.mean().data[0]
          ...
          loss_wsddn_cls = wsddn_loss_cls.data[0]
          loss_wsddn_box = wsddn_loss_bbox.data[0]
          # fg_cnt = torch.sum(roi_labels.data.ne(0))
          # bg_cnt = roi_labels.data.numel() - fg_cnt
        ...
        _print("\t\t\twsddn_cls: %.4f, wsddn_box %.4f" \
                      % (loss_wsddn_cls, loss_wsddn_box), logging)
        if args.use_tfboard:
          info = {
            'loss': loss_temp,
            # 'loss_rpn_cls': loss_rpn_cls,
            # 'loss_rpn_box': loss_rpn_box,
            'loss_rcnn_cls': loss_wsddn_cls,
            'loss_rcnn_box': loss_wsddn_box,
          }
